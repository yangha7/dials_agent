...
def _get_centring_vs_pseudocentring_note() -> str:
    ...
    return (
        ...
        "**The actual decisive test, and a documented failure mode to avoid**: "
        ...
        "whose actual answer is a genuine C222₁ centred setting (R-cryst 0.18/R-free 0.21 vs. "
        "0.27/0.29 for the primitive alternative) that only became visible after reindexing "
        "toward it. The correct sequence when an axis is flagged: (1) `dials.reindex` toward "
        "the centred space group matching that axis; (2) re-run `dials.refine_bravais_settings` "
        "on THAT reindexed result — centred candidates only appear once the input cell's own "
        "metric symmetry admits them; (3) compare real refinement R-factors between the "
        "primitive and centred settings — that comparison decides it, not the I/sigma(I) "
        "reading.\n\n"
