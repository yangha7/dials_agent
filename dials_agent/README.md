...
### v2.15.0 — Fix: `libtbx.python`/`cctbx.python` Fallback Was Silently Rejected, Breaking All Numeric Checks on Installs Without `dials.python`

- **Real bug, caught live on dials.lbl.gov during a full DPF3 auto-processing run**: the user asked to verify why the beam-centre/geometry issue was never identified. Root cause traced from the transcript: `dials.python reciprocal_lattice_linearity.py ...` failed because this install's `conda_base/bin/` has no `dials.python` (confirmed via SSH — it only has `libtbx.python`, `cctbx.python`, `python`). The agent then did exactly what `core/prompts.py` already told it to do — retry with `libtbx.python`, then `cctbx.python` — and both were rejected outright with `"Not a DIALS command"`, never even reaching the executor. So on this install, the reciprocal-lattice-linearity check (and the same would apply to the geometry, centring, and pixel-quality checks) silently never ran at all, three times in a row, and the agent moved on without ever getting a straight answer on beam-centre/geometry.
- Root cause was purely in `dials/commands.py`'s `validate_command()`: it unconditionally rejected any command name not starting with `"dials."`, so the documented, correct fallback path was defeated by validation before execution was ever attempted. This was a pure whitelist bug — `_get_command_path()` in `executor.py` already resolves these correctly once past validation (both binaries do exist at `conda_base/bin/libtbx.python` / `cctbx.python` on this install).
- Fix: added `PYTHON_INTERPRETER_FALLBACKS = {"libtbx.python", "cctbx.python"}` to `commands.py`, checked before the `"dials."`-prefix rule in `validate_command()`. `dials.python` itself was already fine (starts with `dials.`, just unknown -> warns but proceeds); only the two fallbacks needed the allow-list entry. No prompt changes needed — the guidance was already correct, only the code gate was wrong (same "escalate to code" lesson as v2.11.2, applied to a bug this time rather than a UX flow).
- New `tests/test_validate_command.py` (7 tests) covering the fallback allow-list plus confirming arbitrary non-DIALS commands (`rm`, bare `python`) are still rejected. Full suite: 213 passed (`test_mcp_server.py` excluded — `mcp` package not installed in this local dev environment, pre-existing and unrelated).
- Verified on the actual failure case: with this fix, the DPF3 transcript's steps 6-7 (`libtbx.python`/`cctbx.python reciprocal_lattice_linearity.py`) would now execute instead of being rejected, giving the agent the reciprocal-lattice linearity verdict it needed before falling back to `refine_bravais_settings`.
...
- **Honest limitation, consistent with this file's other checks**: unlike the other three (which only need synthetic dxtbx *model* objects, fully testable locally), this one needs real image pixel data via `check_format=True`, which this Mac's local DIALS build can't read due to the same pre-existing dxtbx/nxmx bug noted for the other checks. The pure-numpy statistical logic (sampling, per-image stats, hot-pixel detection, summary text) is fully unit-tested; the dxtbx-integration wrapper (`main()`) is validated by code review only, same as `centring_vs_pseudocentring_check.py`'s and `reciprocal_lattice_linearity.py`'s own precedent
- Wired into `core/prompts.py` (new note, explicitly phrased as opt-in, not automatic) and `data_import/SKILL.md` (mention once after reporting the geometry check, with the runtime warning, only run if asked). Also added to the guaranteed-display mechanism (v2.13.0) so its result is shown directly once it *is* run, extending that mechanism's parser to handle this script's per-panel-nested summary structure
- 216 tests (13 for the pure-numpy logic, 2 for the guaranteed-display extension — one confirming the nested-summary case is found, one incidentally surfacing that Rich's panel rendering can drop wrapped lines under pytest's capsys for multi-line content, an environment quirk unrelated to the actual display logic, worked around by keeping the test's sample text single-line)

### v2.13.2 — `dials.search_beam_position` Restored as an Explicit Option Right After Spot Finding; a Real Correction to `dials.reciprocal_lattice_viewer`'s Documented Scope
- **User confirmed the overall workflow design directly**: the main path should follow the TL;DR tutorial sequence, with everything else offered as branches. Audited `dials/workflow.py`'s `WORKFLOW_SUGGESTIONS` against the TL;DR script end to end — every stage's `next_command` already matched exactly (import→find_spots→index→refine→integrate→symmetry→scale→export/merge, no change needed there)
- **The actual gap**: `spot_finding/SKILL.md`'s LLM-facing numbered options (what interactive users actually see — `optional_commands` in the static data is otherwise never displayed) didn't mention `dials.search_beam_position` at all right after spot finding, even though it was already correctly present in the static data. Fetched the user's referenced SLAC-2026 workflow tutorial directly to confirm exact positioning/framing: "3. Search Beam Position (optional)," between find_spots and index, tied to a visual straightness check in the reciprocal lattice viewer, not a numeric threshold — "for this data set the step is not needed, but it does no harm." Added it as an explicit numbered option at that step
- **A real correction found in the process**: `spot_finding/SKILL.md` previously said not to mention `dials.reciprocal_lattice_viewer` before indexing ("it requires indexed data to show anything meaningful") — the tutorial's own usage contradicts this directly: it runs the viewer on `imported.expt`/`strong.refl` *before* indexing specifically to check for straight lines, since reciprocal-space positions come from pixel coordinates + geometry, not from Miller index assignment. Fixed the incorrect claim in both `spot_finding/SKILL.md` and `dials/commands.py`'s `CommandDefinition` description for this viewer
- 202 tests (1 new, confirming `find_spots`'s optional commands include both `search_beam_position` and `reciprocal_lattice_viewer`)

### v2.13.1 — `dials.refine_bravais_settings` Is Genuinely Optional (Fetched and Confirmed); Fixed the Backwards Report/Next-Step Ordering
- **Two minor points, both real**: (1) the agent was treating `dials.refine_bravais_settings` as a required default step after a flagged geometry bend, when the user's own referenced tutorial (fetched and quoted directly, not assumed) says it's explicitly optional — "in most cases it is absolutely fine to proceed without worrying about the crystal symmetry at this stage," and the tutorial's own minimal/TL;DR path skips it entirely, going straight from `dials.index` to `dials.refine`. (2) after each command, the static "Next Step" preview panel was firing immediately in `display_result()`, before the LLM's own analysis of what was actually learned — so the user saw "here's what to do next" before "here's what we found," backwards from the new Step Transitions policy (v2.13.0)
- **Fix for (1)**: added explicit guidance to `indexing/SKILL.md`'s flagged-bend case — a small static bend explainable by `dials.index`'s own unconstrained-P1 default (not a genuine geometry/beam-centre problem) is expected, not a defect; keep `dials.refine` as the default, offer `refine_bravais_settings` as a named optional alternative, not a required gate. Also softened `dials/workflow.py`'s static "index" suggestion, which previously said refine_bravais_settings "is recommended if you don't already know the space group" — true almost always at this stage, so it read as a de facto default; now matches the tutorial's actual "fine to skip" framing
- **Fix for (2)**: `display_next_step_suggestion()` now takes `only_if_complete`, passed as `True` from `display_result()`'s automatic call — the in-progress "Suggested command" preview stays silent there (letting the LLM's own report-then-options flow handle it, in the right order), while the "Workflow Complete!" banner still fires unconditionally when the workflow genuinely finishes. The explicit `next` command is unaffected — still shows the preview on request
- 201 tests (2 new, confirming the in-progress preview is suppressed from the automatic call but still shown via the explicit `next` command)

### v2.13.0 — Report Numeric Check Findings (Guaranteed), Report + Offer Numbered Options at Every Step Transition
- **User request, broader than any single bug this session**: interactive mode (unlike auto mode) is run live for a workshop/tutorial audience that needs to see what the agent learned at each step, not just get rushed to the next command. Specifically: the numeric checks (import geometry, reciprocal-lattice, centring) were being run but their findings were never reported to the user, and step transitions generally moved too fast without a real chance to look closer
- **Guaranteed fix, not just reworded prompt**: new `DIALSAgent._maybe_display_numeric_check_result()`, called whenever `run_shell_command` runs one of dials_agent's own three bundled numeric-check scripts. It parses the script's own JSON output and prints its verdict/summary directly in a dedicated panel — independent of whether the LLM's own response happens to mention it. This is the part of the request that's fully code-enforced
- **Broader step-transition policy, prompt-based**: replaced `core/prompts.py`'s "Visualization Workflow" section with a new "Step Transitions (Interactive Mode)" section — after every major step, report what happened (including explaining what a numeric check's finding *means*, not just repeating its already-shown raw text) and offer numbered options with "proceed" as the stated default, actually waiting for the answer rather than suggesting the next command in the same breath. Updated `spot_finding`/`indexing`/`refinement`/`integration`/`symmetry`/`data_import`'s SKILL.md files accordingly, since they'd all been written the opposite way (mention-and-move-on) as recently as v2.10.5 — reversing that guidance specifically per this request, not because the earlier design was wrong for what was asked at the time
- **Two explicit, deliberate exceptions preserved**: the CPU-cores default (v2.10.4) and the post-import viewer question (v2.12.0, code-enforced) still don't block the same way — both were separate, explicit prior requests
- Honesty about reliability: the guaranteed-display fix is code-level and can't be skipped. The general "report + ask, then wait" policy is prompt-based, like most of this project's step-by-step guidance, and this session has already seen prompt-only instructions get skipped multiple times for other decisions — worth testing live and reporting back if it doesn't hold up, since escalating this one to a structural mechanism (given how many different steps and follow-up options are involved) would be a larger undertaking than the narrower gates already built for import-choice and viewer-choice
- 199 tests (4 new, covering the guaranteed-display fix: single-crystal, multi-crystal, an unrelated shell command not triggering it, and a failed run not crashing it)
...
- Updated `data_import/SKILL.md` and `core/prompts.py`'s "Offering Options to Users" accordingly: the LLM is now told to just suggest the plain import command directly and not attempt the ask-and-wait itself for this one case, since the CLI now guarantees the real choice happens at the approval gate independent of the LLM's behavior — removing the possibility of a redundant double-ask if the model ever does comply, and removing the failure mode entirely if it doesn't
- `nproc`/CPU-core defaults (v2.10.4) and every other "ask and wait" case remain prompt-only and unaffected — this structural fix is scoped specifically to the one decision point that demonstrated repeated, wording-independent failure
- 192 tests (6 new for `_confirm_command_to_run`, covering both choice branches, cancel, and the three fallthrough cases — already-has-`image_range=`, re-import, and an unrelated command)
